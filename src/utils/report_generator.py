...
def generate_traveler_report(solution):
    ...
    total_jobs = len(
        solution["jobs_completed"]
    )

    total_activities = len(
        solution["activities_completed"]
    )
    ...
    report = {
        ...
        "remaining_budget":
            solution["remaining_budget"],

        "remaining_time":
            solution["remaining_time"],
...
def print_traveler_report(report):
    ...
    for aircraft, usage in (
        report["aircraft_usage"].items()
    ):
...
def generate_graph_statistics(graph):
    ...
    for airport in (
        ...
        for route in airport.routes:
            ...
            for aircraft in (
                route.aircraft_options
            ):

                aircraft_distribution[
                    aircraft.name
                ] = (
                    aircraft_distribution.get(
                        aircraft.name,
                        0
                    ) + 1
                )
